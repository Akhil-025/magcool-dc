...
HONESTY FLAG #1 (book access). This project's copy of Kitanovski et al. (2015) remains the same 30-page front-matter/Chapter-1-only excerpt used throughout Phases 17-19 — it does NOT include Chapter 5 (Magnetocaloric Fluids, pp. 167-209), the chapter the Phase 20 plan itself named as the source for "rheology-adjusted relations" (Sect. 5.2, 5.4, 5.5). None of that chapter's own equations or coefficients could be digitized. A NEW finding from this pass: this project's copy of Tishin & Spichkin (2003) is a scanned, image-only 486-page PDF with NO extractable text layer at all (`pypdf` returned zero characters from every page checked) — its actual chapter contents could not be searched or digitized either, so the Phase 20 plan's own claim that "Tishin doesn't cover this (its Ch. 11 passive-regenerator focus is solid-state)" is taken at face value here, UNVERIFIED by this pass, not independently confirmed one way or the other.

What core/fluid_mce_cycle.py implements instead are three STANDARD, textbook physical relations, none requiring either book: (1) a mixture (lumped) heat-capacity dilution argument for the suspension's effective adiabatic temperature change — ordinary calorimetry, not magnetocaloric-specific; (2) the Krieger-Dougherty equation for suspension viscosity (Krieger & Dougherty, Trans. Soc. Rheol. 3 (1959) 137-152), a standard, widely-used rheology correlation; (3) the Darcy-Weisbach equation for pipe pressure drop, standard textbook fluid mechanics, used instead of core/thermal.py's packed-bed correlations since a continuous-flow fluid loop has no packed bed.

HONESTY FLAG #2 (benchmark availability, found during this pass's own web search — abstracts only, no full-text fetch or digitization of any paper). No clean, directly-usable Qc/COP benchmark for a magnetocaloric-FLUID-as-WORKING-BODY refrigeration device was found in this project's corpus (data/amr_experimental_benchmarks.csv is solid-AMR only) or in this pass's own search. Two ADJACENT, but structurally DIFFERENT, results surfaced and are deliberately NOT conflated with this module's own topic: Andrade, Fernandes, Silva, Teixeira & Pereira (2024), "Magnetic refrigeration enhanced by magnetically-activated thermal switch," couples a SOLID Gd regenerator with a ferrofluid used as a THERMAL SWITCH — a heat-transfer-enhancement/rectification role closer in spirit to Phase 18's thermal-diode item than to this module's working-body topic, and its own abstract reports no COP advantage from the ferrofluid over plain Gd under symmetric cycling; a 2017 gallium-based magnetocaloric liquid-metal ferrofluid study tests cooling performance on an electric transformer coil, not a refrigeration cycle with a comparable reported Qc/COP figure. This module is therefore explicitly a design-exploration/comparison tool, not a validated feature — the same disposition Phase 18 gave core/thermal_diode_analysis.py.

Model. New core/fluid_mce_cycle.py: `krieger_dougherty_viscosity()`, `suspension_effective_properties()` (volume-fraction-weighted density/heat-capacity, defaulting particle properties to magnetite/Fe3O4's standard reference values), `suspension_delta_T_adiabatic()` (the dilution relation — monotonically increasing in phi, always below the pure bulk material's own dTad), `pumping_power_pipe_flow()` (Darcy-Weisbach, laminar/turbulent friction factor switch at Re=2300), and `FerrofluidMCESystem` — a new sibling class to AMRSystem with its own `cooling_capacity()`, `magnetic_work()`, `run()`, `characteristic_curve()`. Structurally, `cooling_capacity()` uses a FULL-swing span-fraction denominator (`1 - T_span/dTad_suspension`), not AMRSystem's regenerated half-swing denominator (`1 - T_span/(2*dTad)`) — a genuine consequence of having no regenerator to amplify achievable span beyond a single adiabatic stroke, documented directly in the module's own docstring rather than silently reused from AMRSystem. `magnetic_work()` uses a fixed, explicitly UNCALIBRATED `eta_2nd_law_fluid` (default 0.20, deliberately lower than AMRSystem's literature-anchored 0.35-0.52 range) since no benchmark device exists to calibrate it against (HONESTY FLAG #2).

Validation/exploration. New core/fluid_mce_analysis.py: `volume_fraction_sweep()` sweeps particle_volume_fraction at a fixed representative operating point, evaluating each phi at ITS OWN self-consistently-solved favorable span (a fixed-point loop, `_self_consistent_span()`, solving span = 0.9*dTad(T_cold + span/2) — a real bug was found and fixed during this pass: naively evaluating dTad at an arbitrary fixed T_mid before computing span produced a T_mid/span inconsistency that silently zeroed out Qc for every row, since Gd's dTad(T) is sharply peaked and even a few Kelvin of T_mid mismatch mattered at these sub-Kelvin-to-few-Kelvin span scales). `compare_to_solid_amr_and_liquid_cooling()` compares FerrofluidMCESystem against a representative solid AMRSystem (Gd, same field/flow) and against core.baseline_cooling's liquid-cooling and vapor-compression references, at the fluid system's own favorable span, per the plan's own framing ("worth at least a first-pass comparison against your existing direct-liquid-cooling baseline, since it's the closest architecture your competitor technology already uses"). `run_magnet_geometry_analysis()`-style `run_fluid_mce_analysis()` orchestrates both and writes results/fluid_mce_analysis.txt.

Findings (genuine, unforced — reported as computed, not massaged). (1) A real interior optimum in COP_electrical vs. particle volume fraction was found in this pass's own run (best near phi≈0.10-0.20, depending on exact sweep grid), confirming the viscosity-vs-phi / MCE-intensity-vs-phi tradeoff the plan named is a genuine tradeoff in this model, not a monotonic result at either boundary. (2) The headline finding: at realistic particle loadings (phi up to phi_max=0.63, random-close-packing), the mixture-heat-capacity dilution model combined with this architecture's lack of regeneration collapses the usable span to well under a Kelvin up to a few Kelvin — dramatically less than solid AMR achieves at the same field/flow, whose regenerator bed amplifies achievable span well beyond a single stage's own dTad. (3) A narrower, secondary finding reported carefully rather than overstated: at the fluid system's own shared tiny span, its own COP_electrical can actually exceed solid AMR's (because the small Darcy-Weisbach pumping-power term is a much smaller fraction of total work than AMR's parasitic-loss terms at that operating point, and Qc largely cancels out of the COP ratio for both systems) — but BOTH trail the liquid-cooling and vapor-compression baselines at that same tiny span, and this COP comparison should not be read as "ferrofluid MCE beats solid AMR" in general, since the solid AMR could trivially also hit that same tiny span with far more absolute cooling capacity.

Integration points. main.py: new imports `from core import fluid_mce_analysis`; new step "14. Magnetocaloric fluids (ferrofluid/MR suspension) as an alternative working-body class (core/fluid_mce_cycle.py, core/fluid_mce_analysis.py, Phase 20)" runs `fluid_mce_analysis.run_fluid_mce_analysis()` and captures `fluid_mce_result` for the executive summary (new elif branch, new `fluid_mce_result = None` initializer, `_print_executive_summary()`'s signature extended with a `fluid_mce_result` parameter and a new summary block reporting the volume-fraction-sweep optimum and the span/Qc headline comparison); results/ file listing updated (fluid_mce_analysis.txt).

Tests added: tests/test_fluid_mce_cycle.py (14 tests — Krieger-Dougherty monotonicity/divergence-near-phi_max/input-validation, suspension property bounds, dilution-factor monotonicity, dTad-below-pure-material bound, dTad=0 at phi=0, pumping-power zero-flow/viscosity-scaling checks, FerrofluidMCESystem input validation, a sane-result smoke test, the no-regeneration "Qc falls to zero beyond dTad span" structural property directly asserted, characteristic-curve shape, carrier-fluid validation), tests/test_fluid_mce_analysis.py (6 tests — sweep returns all rows, span grows monotonically with phi, comparison returns all four technologies with positive figures, solid AMR delivers more absolute cooling capacity than the fluid system at the shared span, file-writing and no-file-write smoke tests). Full suite: 313/313 passing (293 pre-Phase-20 + 20 new: 14+6), zero regressions (verified in three batches due to this session's per-command time budget: 249 passed in the non-NSGA/plots subset, 34 passed in the NSGA-III/magnet-geometry subset, 30 passed in the plots/thermal-diode subset).
...
HONESTY FLAG (book access — same tier as Phases 17-20's own flags). The plan's stated data sources were Tishin & Spichkin (2003) Sect. 11.1 (passive magnetic regenerators in conventional gas-cycle refrigerators), Sect. 11.2.3 (magnetically-augmented gas regenerators) and Sect. 11.2.4 (hybrid magnetic working bodies). As already found and documented in Phase 20's own ROADMAP.md entry, this project's copy of Tishin & Spichkin (2003) is a scanned, image-only PDF with NO extractable text layer — re-confirmed for this pass, not assumed. None of those sections' own equations, reported effectiveness-vs-alignment curves, or COP figures could be digitized. What is implemented instead is, exactly as the plan itself anticipated, a reuse of this repo's own existing data recombined in a new way, not a reproduction of Tishin's own numbers.

Model. core/thermal.py's `regenerator_effectiveness()` gained a new, backward-compatible `cp_solid=None` parameter (default reproduces the exact pre-Phase-21 behavior — the fixed module-level CP_SOLID_GD — for every existing caller; same additive-override discipline as Phase 15's `pumping_power_override`, Phase 17's `cycle_type="brayton"`, and Phase 18's `thermal_diode=None`). core/baseline_cooling.py gained the Phase 21 deliverable, named and shaped exactly as the plan specified: `augmented_regenerator_cop(base_cop, passive_regenerator_material, T_range)`, built on a new `passive_regenerator_augmentation()` helper and a new `PassiveRegeneratorResult` dataclass. The mechanism: a candidate material's own `total_heat_capacity()` (lattice + magnetic lambda-anomaly) is averaged over the operating window `[T_cold, T_hot]` and compared, via `regenerator_effectiveness()`'s new `cp_solid` override, against the SAME material's own `lattice_heat_capacity()` averaged over the same window — isolating the magnetic-anomaly contribution from bulk lattice properties (see IMPLEMENTATION NOTE below for why this specific comparison, not a shared flat reference, was chosen). The resulting effectiveness gain (`delta_eps`, clipped at 0 — a poorly-aligned material must reduce to the unaugmented baseline, never make it worse) is mapped to a fractional COP gain via a single illustrative ceiling, `MAX_COP_GAIN_AT_FULL_EFFECTIVENESS = 0.08` (8%), loosely anchored to the generically-reported few-percent-to-~10% COP improvement from internal (liquid-suction) heat exchangers in the vapor-compression literature — explicitly NOT a fitted or digitized coefficient, same evidentiary tier as Phase 16's `hysteresis_loss_J_per_kg` and Phase 18's `actuation_energy_J_per_cycle` placeholders.

IMPLEMENTATION NOTE (a real correction made during this pass, stated rather than left implicit). An early version of `passive_regenerator_augmentation()` compared each candidate material's full (lattice + magnetic) heat capacity against one shared flat constant (CP_SOLID_GD) applied identically across every material. Running the actual numbers exposed that this produced a materially wrong ranking: La0.7Ca0.3MnO3 (5 atoms per formula unit, a much larger Dulong-Petit lattice heat capacity per kg than Gd purely from its heavier, multi-atom unit cell) ranked ABOVE Gd even though Gd's own Curie temperature (294K) sits inside the representative [291.15K, 301.15K] operating window and La0.7Ca0.3MnO3's (267K) does not — the flat-constant comparison was mostly measuring bulk lattice properties, not magnetic alignment. The fix (each material compared against its OWN lattice-only baseline, described above) is what is documented and tested; the broken flat-reference version was never committed to results/ or ROADMAP.md as a finding.

Validation/exploration. New core/passive_regenerator_analysis.py: `compare_candidate_materials()` runs `augmented_regenerator_cop()` for GADOLINIUM, GD5SI2GE2 (its mean-field parameterization — first-order materials from core/first_order_mce.py are deliberately excluded, since that class has no Curie-point heat-capacity peak to exploit; see the module's own docstring), and LACAMNO3 at the representative ASHRAE operating point (T_cold=291.15K, span=10K, matching main.py's own REPRESENTATIVE_SPAN_K); `span_sweep()` repeats the comparison across spans=[5,10,15,20]K at fixed T_cold to directly check (not assume) that the gain moves as the operating window's overlap with each material's own Tc changes. `run_passive_regenerator_analysis()` writes results/passive_regenerator_analysis.txt.

Findings (genuine, unforced — reported as computed). At the representative point, Gd gives the largest boost (eps 0.829 -> 0.860, COP 12.228 -> 12.258, +0.24%) since its own Tc=294K sits inside the operating window; Gd5Si2Ge2 (Tc=276K, mean-field parameterization) and La0.7Ca0.3MnO3 (Tc=267K) — both well outside the window — give ~0% in this run. The gain shrinks monotonically as span widens at fixed T_cold (5K: +0.40% -> 20K: +0.14%), consistent with a wider window diluting the averaged heat-capacity boost around a fixed Tc — a genuine, checked alignment effect in this repo's own model, not asserted from the mechanism's description alone.

Integration points. main.py: new import `from core import passive_regenerator_analysis`; new step "15. Passive/hybrid magnetic regenerator augmentation of a conventional gas cycle (core/baseline_cooling.py, core/passive_regenerator_analysis.py, Phase 21)" runs `passive_regenerator_analysis.run_passive_regenerator_analysis()` and captures `passive_regen_result` for the executive summary (new elif branch, new `passive_regen_result = None` initializer, `_print_executive_summary()`'s signature extended with a `passive_regen_result` parameter and a new summary block reporting the best candidate and the alignment-effect finding); results/ file listing updated (passive_regenerator_analysis.txt). Full `python main.py` run confirmed clean end-to-end (all 26 stages "ok", 161.9s total wall time), with step 15's new output present in both the stage log and the executive summary.

Tests added: tests/test_thermal.py (2 new — `cp_solid=None` exactly reproduces the pre-Phase-21 default, a higher `cp_solid` override lowers utilization U and raises or preserves eps), tests/test_baseline_cooling.py (5 new — an aligned material shows a strictly larger effectiveness gain than a misaligned one, `augmented_regenerator_cop()` never returns a COP below the base COP even for a badly-misaligned material, the fractional COP gain never exceeds the illustrative ceiling, the returned dataclass's fields are internally consistent, an explicit `cp_solid_baseline` override still works), tests/test_passive_regenerator_analysis.py (6 new — all three candidate materials are returned, results are sorted descending by augmented COP, Gd ranks first at the representative point (the concrete, checked instance of the alignment claim), the span sweep returns one row per span, `run_passive_regenerator_analysis()` writes its output file and returns a well-formed dict, and works with `out_path=None`). Full suite: 326/326 passing (313 pre-Phase-21 + 13 new: 2+5+6), zero regressions (verified in four batches due to this session's per-command time budget: 280 passed in the non-NSGA/non-plots/non-Astronautics subset — including all 13 new tests, 15 passed in the NSGA-III/optimize subset, 6 passed in the Astronautics-graded-bed subset, 27 passed in the plots subset; `pytest --collect-only` independently confirms 326 tests collected).
...
HONESTY FLAG (book access — same tier as Phases 17-21's own flags). Tishin & Spichkin (2003) Sect. 2.8 is this item's own named primary source. Re-confirmed directly for this pass (not assumed from Phase 20/21's prior finding): pdfplumber extracts zero characters from every one of the book's 486 pages sampled (0, 1, 2, 50, 51) — the same image-only-PDF finding Phases 20-21 already recorded for Tishin Ch. 11, now independently re-checked against Sect. 2.8's own page range. Sect. 2.8's specific content — whatever broadening model or fitted widths it reports — could not be read or digitized. What is implemented instead is the standard literature treatment of inhomogeneous broadening for a second-order magnetic transition (a Gaussian-distributed local grain Curie temperature, per the general mean-field near-Tc literature already cited in core/validation.py for this same model's other limitations — de Oliveira & von Ranke, Phys. Rep. 489 (2010) 89-159), not a reproduction of Sect. 2.8's own numbers.

Model. New core/mce_material.py method `MagnetocaloricMaterial.with_Tc(new_Tc)` — a `dataclasses.replace()` clone sharing every parameter except Tc (and its derived Weiss constant, recomputed by `__post_init__`). New core/inhomogeneous_broadening.py: `BroadenedMagnetocaloricMaterial(base_material, sigma_Tc_K, n_quad=15)` builds a Gauss-Hermite-quadrature ensemble of `with_Tc()` clones representing grains with locally-distributed Tc ~ N(Tc_bulk, sigma_Tc^2), all sharing the same bulk T and applied H (grain-to-grain field-penetration/demagnetization differences are not modeled — a stated simplification). The sample's entropy and heat capacity are volume-fraction-weighted sums over the ensemble (`entropy_magnetic()`, `total_heat_capacity()`), fed through the SAME small-signal `DeltaT_ad = -T*DeltaS/C` approximation the base (unbroadened) model already uses — broadening only changes what goes into S and C, not the approximation linking them. numpy's probabilist Hermite quadrature (`hermegauss`) is exact for a Gaussian weight; 15 nodes match a brute-force 2001-point linspace/trapz reference to ~0.1-0.5% relative (tests/test_inhomogeneous_broadening.py::test_quadrature_matches_brute_force_reference), comfortably tighter than the several-percent-scale physics this sensitivity study is already working with.

Calibration (genuinely open — stated rather than papered over). No digitized experimental DeltaT_ad(T) CURVE exists anywhere in this repo's corpus (core/validation.py's Dan'kov et al. numbers are point values near the peak at three fixed fields, not a curve), so sigma_Tc cannot be fit by nonlinear regression against real data here. Instead `run_inhomogeneous_broadening_analysis()` SWEEPS sigma_Tc over 0-5 K (a physically plausible order of magnitude for grain-to-grain Tc variation in polycrystalline rare-earth samples of varying purity) and reports the direction of the trend, not a fitted final value.

Findings (genuine, unforced — reported as computed, including the parts that don't look like a clean win). At T=294K (core/validation.py's own fixed-T methodology, for direct comparability), broadening lowers the 1T/2T overestimate the sharp model already has (+48.9% -> +20.9% at 1T, sigma_Tc=5K) but WIDENS the 5T underestimate the other direction (-7.5% -> -14.2%) — a real trade-off, not a uniform improvement: broadening globally smooths the transition, it does not selectively fix the field-dependent error PATTERN. The worst-field |error| across the three Dan'kov fields does fall monotonically over the whole 0-5K sweep (48.9% -> 45.6% -> 42.4% -> 36.3% -> 30.7% -> 20.9%), but the minimum sits at the LARGEST sigma_Tc tested (5K) with the trend still falling at that edge — this sweep has NOT located an interior optimum, only shown a direction, and says so explicitly in its own printed conclusion rather than reporting sigma_Tc=5K as "the answer." At fixed 5T, sigma_Tc=5K widens the DeltaT_ad(T) FWHM from 31.4K (sharp) to 45.4K, confirming broadening does what it is supposed to (widen the transition) even though it does not cleanly resolve the model's field-dependent miscalibration. This is additional, first-pass evidence for the SAME near-Tc mean-field limitation core/validation.py's own `run_curie_shift_check()` already documents from a different angle (the sharp model's peak temperature does not shift with field the way Dan'kov et al. report) — not a contradiction of it, and not a fix for it either.

Integration points. core/mce_material.py: new `with_Tc()` method (additive, no existing call sites touched). main.py: new import `from core import inhomogeneous_broadening`; new step "1b. Inhomogeneous/polycrystalline Tc-broadening sensitivity (core/inhomogeneous_broadening.py, Phase 22 item 1)" runs immediately after step 1 (material-level validation) since it directly extends that same Dan'kov et al. comparison — dispatched through the existing generic `else: fn()` branch in `main()`'s stage loop (no special-case elif needed, since the stage needs no captured return value fed to a later step); module docstring's numbered stage list updated. results/ file listing gains results/inhomogeneous_broadening.txt.

Tests added: tests/test_inhomogeneous_broadening.py (11 new — zero-sigma ensemble degenerates to the base material with weight 1, Gauss-Hermite weights sum to 1 for several sigma values, the weighted-mean Tc of the quadrature nodes reproduces the base material's own Tc, `with_Tc()` preserves every other parameter, a tiny-sigma broadened material converges to the sharp model's own DeltaT_ad, 15-node quadrature matches a brute-force 2001-point reference to within 0.5% relative, broadening widens (never narrows) the DeltaT_ad(T) FWHM and never raises the peak, the sweep helper returns well-formed rows, sigma_Tc=0 rows reproduce core/validation.py's own `run_validation()` numbers exactly, the full top-level analysis runs end-to-end and writes its report file, and the reported best_sigma's worst-field error is bounded by the sharp model's own worst-field error by construction). Full suite: 337/337 passing (326 pre-Phase-22 + 11 new), zero regressions — verified in four batches due to this session's per-command time budget, same split Phase 21 used: 283 passed (everything except NSGA-III/optimize, plots, and cascade — includes all 11 new tests), 10 passed (NSGA-III/optimize subset), 17 passed (cascade/Astronautics-graded-bed subset), 27 passed (plots subset); `pytest --collect-only` independently confirms 337 tests collected.
...
HONESTY FLAG (book access -- same tier as Phases 17-22 item 1's own flags). Tishin & Spichkin (2003) Sect. 2.9 and Ch. 10 (superparamagnetic/nanocomposite/molecular-cluster systems) are this item's own named sources per phase_plan.md. Re-confirmed directly for this pass: pdfplumber extracts zero characters from every page of this project's copy sampled (0, 1, 2, 50, 51) -- the same image-only-PDF finding already recorded for Tishin Ch. 11 (Phase 21) and Sect. 2.8 (Phase 22 item 1). Those sections' specific content could not be read or digitized here. Per phase_plan.md's own flagged limitation, this item does NOT invent new composition/Tc/DeltaS_M data for a true superparamagnetic nanoparticle system (no such data exists anywhere in this repo's corpus), and does NOT attempt true superparamagnetic physics (Neel relaxation, a blocking-temperature-dependent effective-moment reduction, particle-size-distribution effects) -- that would need particle-size-distribution and anisotropy-constant data this project does not have. Instead, a narrower, well-established reading of "nanocomposite" is implemented: the broader MCE engineering literature (e.g. Smith et al., "Materials challenges for high performance magnetocaloric refrigeration devices", Adv. Energy Mater. 2 (2012) 1288-1318) documents engineered composite regenerators -- blending multiple composition-tuned phases of the SAME base material family, each with a slightly different Tc, within a single layer to broaden/flatten the working temperature range -- as a real design strategy, distinct from core/cascade.py's existing discrete-stage Curie grading. This module implements exactly that, using only ALREADY-VALIDATED, already-tunable LAFESIH_FAMILY phases -- no new undigitized material data.

Model. New core/nanocomposite_material.py: `WeightedMaterialEnsemble(materials, weights, name=None)` -- a fixed, explicit weighted blend of heterogeneous material objects (anything implementing the `delta_T_adiabatic(T, H_final, H_initial=0.0)` interface shared by MagnetocaloricMaterial and FirstOrderMCEMaterial). Unlike Phase 22 item 1's BroadenedMagnetocaloricMaterial (which averages entropy and heat capacity separately, correct for item 1's C_lattice-only mean-field model), this ensemble mixes at the DeltaT_ad level directly -- `DeltaT_ad_ensemble(T) = sum_i w_i * DeltaT_ad_phase_i(T)` -- because FirstOrderMCEMaterial's own `dTad_correction` (calibrated against Giguere et al.'s direct measurement, see core/giguere_validation.py) is a correction to the whole DeltaT_ad ratio, not to entropy or heat capacity individually; averaging entropy/heat-capacity separately would silently discard that per-phase correction. This is a real, stated methodological difference between the two ensemble types in this repo, not an oversight. `nanocomposite_tuned_material(Tc_center_K, spread_K=4.0, weights=(0.25, 0.5, 0.25))` builds a 3-phase triangular-weighted blend of `lafesih_composition_tuned_material()` calls at Tc_center-spread, Tc_center, Tc_center+spread. `NANOCOMPOSITE_FAMILY` (a `core.cascade.GradedFamily`) wraps this with `tc_min`/`tc_max` tightened by `spread_K` on each side of LAFESIH_FAMILY's own documented window (190-340K), so every phase always stays within LAFESIH's own validated tunability range -- no per-phase fallback logic needed. `NANOCOMPOSITE_SPREAD_K=4.0` is an illustrative, design-chosen value (no experimental nanocomposite curve exists in this repo's corpus to fit against -- stated directly, not worked around), deliberately narrower than item 1's swept 0-5K random-inhomogeneity range since it represents an intentional design choice, not an unavoidable manufacturing side-effect.

Verification before wiring in. Confirmed numerically (not assumed) that peak_T(Tc_center) is monotonically increasing across the nanocomposite family's own tc window, the same property core/cascade.py's own `_target_composition_for_peak()` docstring says was separately verified for GD_FAMILY/LAFESIH_FAMILY -- required for that function's bracketed root-finder to converge. Confirmed `_target_composition_for_peak()` converges correctly for NANOCOMPOSITE_FAMILY and reproduces a target peak temperature to within 0.5K.

Integration points. core/material_family_comparison.py: `NANOCOMPOSITE_FAMILY` added to `TUNABLE_FAMILIES`, module docstring updated from "five candidates" to "six candidates", header text updated from "Four-way" to "Six-way". core/plots.py: Fig 26's `colors5` list (5 entries) extended to `colors6` (6 entries, defensively re-sliced/tiled to match candidate count exactly, with a regression-test cross-reference in the comment); header comment updated. main.py: new import `from core import nanocomposite_material`; stage "8d." relabeled "Six-way material family comparison ... + Phase 22 item 2"; new stage "8e. Nanocomposite off-design robustness check (core/nanocomposite_material.py, Phase 22 item 2 follow-up)" added directly after it, dispatched through the existing generic `else: fn()` branch (no special-case elif needed).

Finding 1 (point performance, material_family_comparison.py's existing per-span-retuned methodology). At every span checked, the nanocomposite underperforms a perfectly single-phase-tuned LAFESIH material at that SAME span's own target Tc -- e.g. at 5K span, nanocomposite COP=6.54/Qc=2525.5W vs. single-phase LAFESIH COP=8.92/Qc=5752.7W. This is the expected, unforced direction (blending trades peak height for width) and is pinned by tests/test_nanocomposite_material.py::test_nanocomposite_underperforms_perfectly_tuned_single_phase_at_design_point. The nanocomposite ranks 3rd of the 4 rankable (non-infeasible) candidates at the representative 10K span, behind LAFESIH (tuned) and plain Gd, ahead of the Gd-family tuned candidate.

Finding 2 (off-design robustness, new `run_robustness_check()` follow-up -- genuinely checked, not assumed). Because Finding 1's per-span-retuned comparison cannot show whether a blend's breadth buys anything (a perfectly retuned single phase always wins at its own design point by construction), `run_robustness_check()` instead builds ONE nanocomposite and ONE single LAFESIH phase, BOTH tuned once at a 10K-span design point, then evaluates both WITHOUT retuning across spans 5/10/15/20K. Result: at the design span (10K) the single phase wins outright, as Finding 1 already established. At the 15K and 20K off-design spans, BOTH candidates collapse to Qc=0 (too far off-design for either). But at the 5K off-design span (narrower than the design span), the single phase's sharply-tuned response collapses to Qc=0 while the nanocomposite still delivers Qc=863.9W -- a genuine, checkable robustness benefit: the nanocomposite's deliberately widened working range covers an off-design span the sharply-tuned single phase cannot reach at all, even though it never wins on raw Qc/COP anywhere both remain feasible. This is reported as a first-pass finding at one spread value and one span set, not a general claim that composite regeneration is always worth it -- stated explicitly in the module's own printed conclusion. Written to results/nanocomposite_robustness.txt.

Tests added: tests/test_nanocomposite_material.py (13 new -- weight-sum validation rejects malformed inputs, ensemble Tc is the weighted mean, a degenerate 1-material/weight-1.0 ensemble reproduces that material's own delta_T_adiabatic exactly, delta_T_adiabatic mixing matches a hand-computed linear combination, hysteresis_loss_J_per_kg is a weighted average, nanocomposite_tuned_material builds phases at the expected three Tc values, the tc window is tightened by spread_K exactly as documented, boundary Tc values do not raise, peak_T(Tc_center) is monotonic across the family's own window, the root-finder converges to within 0.5K, the nanocomposite underperforms a perfectly-tuned single phase at the design point (regression guard on the qualitative direction), the robustness check returns a well-formed result and writes its report file, and core.cascade.run_cascade accepts a WeightedMaterialEnsemble directly with no special-casing). tests/test_material_family_comparison.py updated: the "five candidates" cardinality test renamed/updated to assert six. tests/test_plots.py: one new test (`test_plot_material_family_comparison_color_count_matches_candidates`) added as the regression guard core/plots.py's own comment references, confirming the colors6 list never falls short of the candidate count and the figure function runs without raising. Full suite: 351/351 passing (337 pre-item-2 + 13 nanocomposite + 1 plots) -- verified in two batches this session (296 passed: everything except NSGA-III/optimize, plots, and cascade; 10 passed: NSGA-III/optimize subset) plus cascade (17) and plots (28) independently confirmed passing earlier in this same session after the respective edits; `pytest --collect-only` independently confirms 351 tests collected, and 296+10+17+28=351 reconciles exactly against that count.
...
HONESTY FLAG (book access -- same tier as Phases 17-22's own flags, checked for completeness even though no model was built from it). Tishin & Spichkin (2003) Sect. 2.10 (magnetoelastic/anisotropic contributions) and Ch. 9 (amorphous materials) are this item's own named sources. Re-confirmed directly for this pass: pdfplumber extracts zero characters from every page of this project's copy sampled (0, 1, 2, 50, 51) -- the same image-only-PDF finding already recorded for Ch. 11 (Phase 21), Sect. 2.8 (Phase 22 item 1), and Sect. 2.9/Ch. 10 (Phase 22 item 2). Confirmed for the record rather than skipped, since even a "no model" decision should be checked against real book access rather than assumed.

Magnetoelastic/anisotropic contributions (Sect. 2.10): deliberately NOT modeled. phase_plan.md itself does not name a deliverable for this half of the item (contrast with the amorphous half, which explicitly asks for a note), and no other part of this repo's own documented gaps (ROADMAP.md, giguere_validation.py's honesty flags, etc.) independently motivates it the way Phase 22 item 1's broadening gap or item 2's material-family slot did. Building even a first-pass magnetoelastic/anisotropy correction would require single-crystal anisotropy-constant and magnetostriction data this repo's corpus does not have (and Sect. 2.10 itself is unreadable, per the honesty flag above), so there is no way to do this "cheaply" the way items 1-2 were scoped to be done cheaply. This is recorded here as a conscious, plan-consistent decision not to build something, not an oversight.

Amorphous materials (Ch. 9): implemented exactly as scoped -- a one-line-note addition to core/economics.py, not a materials model. New section: `AMORPHOUS_MATERIAL_COST_PERFORMANCE_NOTE` (a qualitative string) and `amorphous_material_cost_performance_note()` (returns it), added immediately after `resource_criticality_note()` (Phase 15), matching that function's own established pattern (a qualitative note function, not a numeric model, with an explicit "not assessed"/no-fabricated-number stance). Content: amorphous (melt-spun ribbon / metallic-glass) MCM candidates trade lower manufacturing cost (melt-spinning is a single continuous step, skipping the slow single-crystal-growth/annealing this repo's already-priced crystalline families need) for lower peak DeltaS_M/DeltaT_ad -- the same structural disorder that makes them cheap to produce also broadens and shallows the magnetic transition, explicitly framed as the SAME qualitative peak-vs-width trade-off Phase 22 items 1-2 already quantify numerically for two OTHER broadening mechanisms in this repo (random grain-to-grain Tc inhomogeneity, deliberate multi-phase blending) -- amorphous structural disorder is presented as a third, distinct source of the same general trade-off, not a new mechanism modeled separately here. No amorphous-MCM $/kg or DeltaS_M figure is digitized anywhere in this repo's corpus, so none is invented: the note is explicitly NOT wired into `MCM_COST_PER_KG_BY_FAMILY` or any cost/performance calculation, consistent with how that dict's own comment already declines to invent a number for MNFEPSI_FAMILY's/GD_FAMILY's own missing $/kg figures.

Integration points. core/economics.py: new section (after `resource_criticality_note()`, before the Phase 19 geometric-magnet-mass section) with `AMORPHOUS_MATERIAL_COST_PERFORMANCE_NOTE` and `amorphous_material_cost_performance_note()`. main.py: `run_full_system_cost_by_material()` (step 5b) now also logs the note after its existing per-family cost table, so it surfaces in every full pipeline run without a dedicated new stage (unlike items 1-2, which each got their own numbered stage, this is intentionally folded into the existing cost step since it IS just a note, not a new analysis); step 5b's label updated to mention Phase 22 item 3.

Tests added: tests/test_economics.py gains 2 new tests -- `test_amorphous_material_cost_performance_note_is_qualitative_only` (non-empty string, mentions "amorphous", and confirms `MCM_COST_PER_KG_BY_FAMILY`'s own keys do NOT contain "amorphous" -- i.e. the note is genuinely not wired into the priced-families dict) and `test_amorphous_material_cost_performance_note_is_stable_across_calls` (deterministic, no hidden randomness). Full suite: 353/353 passing (351 pre-item-3 + 2 new) -- verified in batches this session: 298 passed (everything except NSGA-III/optimize, plots, and cascade -- includes both new economics tests), 10 passed (NSGA-III/optimize) plus 6 passed (test_main.py, re-checked after the main.py edit), plus cascade (17) and plots (28) independently confirmed passing earlier in this same overall Phase 22 effort and unaffected by this item's changes; `pytest --collect-only` independently confirms 353 tests collected, and 298+10+17+28=353 reconciles exactly against that count (test_main.py's 6 are already included within the 298 batch's "everything except..." scope, so are not added a second time).
...
HONESTY FLAG (book access — same tier as Phases 17-22's own flags, and the reason this phase's numbers come from neither of this repo's two source books). phase_plan.md's own Phase 23 data source is Kitanovski et al. (2015) Ch. 10 §10.3 ("Elastocaloric Energy Conversion", pp. 438-446 per that book's own table of contents, visible in this repo's copy). Checked directly for this pass: this project's copy of Kitanovski et al. (2015) is a 30-page excerpt — cover, front matter, preface, the full table of contents, and the opening pages of Chapter 1 only (confirmed by pdfplumber's own extracted page count, 30, against the book's real ~470). Chapter 10 is simply not present in the file this repo has access to; nothing from it could be read or digitized. Tishin & Spichkin (2003) does not cover elastocalorics at all (the book predates the field's modern development and has no elastocaloric chapter in its own table of contents, already established across Phases 20-22's own honesty flags) and is separately confirmed image-only with no text layer. So, exactly as phase_plan.md's own Phase 23 entry anticipated, the reference figures below come from external, independently-published, peer-reviewed literature located by this pass's own search — not from either of this repo's two source books, and not a reproduction of either book's own numbers (it couldn't be, since neither is readable here).

Sourced anchors (device/SYSTEM-level COP — the figure directly comparable to this repo's own `COP_electrical`, not the narrower material-level `COP_mat` figure-of-merit some elastocaloric papers report separately): Qian, Catalini, Muehlbauer, Liu, Mevada, Hou, Hwang, Radermacher & Takeuchi, "High-performance multimode elastocaloric cooling system", Science 380, 722-727 (2023) — simulated steady-state system COP = 5.8, at up to a 22.5K span (the closest published anchor to this repo's own 5-20K ASHRAE sweep; the paper's own initial hardware measurements did not yet capture every system loss, so the authors themselves flag this simulated figure as the one still needing experimental verification — stated here rather than smoothed over); Wu et al., "Continuous and efficient elastocaloric air cooling by coil-bending", Nat. Commun. 14, 7982 (2023) — MEASURED device-level system COP = 3.7, but at a much narrower ~1K temperature drop, a genuinely different operating regime than this repo's own span sweep, so it is kept as the LOW end of a reported range rather than averaged in as an equal-weight point.

Model. New section in core/baseline_cooling.py: `ElastocaloricReferenceResult` dataclass and `elastocaloric_reference_cop()`, named and shaped exactly as the plan specified — a lookup, not a simulation, taking no span/temperature arguments. `COP_representative` is the geometric mean of the two sourced anchors (chosen over an arithmetic mean since COP is a ratio-like efficiency figure). Deliberately NOT built as a span-dependent model: neither literature anchor reports a COP(span) curve over this repo's own 5-20K sweep range (Qian et al.'s own device reaches spans up to 22.5K but at one reported operating COP, not a swept curve; Wu et al.'s COP=3.7 figure is reported at a ~1K span) — fitting a COP(span) curve from two single-point anchors measured at non-overlapping spans would invent a slope this repo has no data for, so the function returns one flat reference value plus the `[COP_low, COP_high]` range it was drawn from, honestly representing the actual precision of the evidence.

Integration points. core/plots.py fig08 (`plot_amr_vs_baselines()`): imports `elastocaloric_reference_cop`; adds a fourth, flat horizontal reference line (dashed, `#8e44ad`) plus a shaded `[COP_low, COP_high]` band via `ax.axhspan()`, plotted across the same span axis as the three existing span-dependent curves — reusing `precomputed['baseline_rows']`'s new `Elastocaloric_COP_ref` column when available (same precomputed-reuse pattern the function's own docstring already documents for the other three series), falling back to a freshly-computed flat list otherwise so the no-argument call path (exercised directly by tests/test_plots.py) is unaffected. main.py step 4 (`run_baseline_sweep()`): imports `elastocaloric_reference_cop`; computes it once (not per-span, consistent with it being a static reference); every row of results/comparison_table.csv gains a repeated `Elastocaloric_COP_ref` column; the per-span log table gains a matching column, and a new one-line log statement reports the representative value, its literature range, and its full source note immediately after the table — so the honesty flag is visible in every pipeline run's own log, not only in source-code comments. `_print_executive_summary()`'s existing baseline-comparison block gains one new line reporting the Phase 23 reference COP alongside the existing AMR/VCC/liquid/Carnot figures, explicitly labeled "static literature reference, NOT span-simulated."

Tests added: tests/test_baseline_cooling.py (3 new — `elastocaloric_reference_cop()` returns the expected dataclass shape with the two sourced literature bounds and a source note that says "literature"; `COP_representative` is confirmed to be the geometric mean of `ELASTOCALORIC_COP_LOW`/`ELASTOCALORIC_COP_HIGH` and to fall strictly inside that range; the function is confirmed callable with no arguments and to return an identical, positive value across repeated calls — a direct regression guard on it being genuinely static rather than secretly reading mutable state). tests/test_plots.py required no new test: the existing generic `test_figure_function_runs_and_saves_png_and_pdf` parametrization already exercises `plot_amr_vs_baselines` with no precomputed rows, which now also covers the new elastocaloric-line code path's no-argument fallback branch. Full suite: 356/356 passing (353 pre-Phase-23 + 3 new), zero regressions — verified in two batches: 328 passed (everything except plots.py, including all 3 new baseline_cooling tests and the full core suite with SALib/pymoo installed), 28 passed (full plots.py suite, including fig08's regenerated figure, independently re-rendered and visually confirmed to show the new dashed reference line and shaded literature-range band sitting in the same COP neighborhood as the AMR curve across the swept span range).

What this phase deliberately did NOT do: did not digitize Kitanovski Ch. 10 §10.3's own content (not available — see honesty flag above), so the reference figures are drawn from independent peer-reviewed literature this pass located directly, not a reproduction of that section's own numbers or examples. Did not build a span-dependent elastocaloric COP model or simulate an elastocaloric AMR-analog cycle the way core/amr_cycle.py simulates magnetic AMR — phase_plan.md's own scoping explicitly asks for a static reference point, not a new device model, and no literature source found in this pass reports a COP(span) curve over this repo's own 5-20K range to fit one against. Did not add an elastocaloric row to core/economics.py's or core/emissions.py's cost/GWP comparisons — the plan names only a plots.py/baseline_cooling.py comparison-line deliverable for this phase, and a cost/emissions figure for elastocaloric NiTi-based systems would need its own sourced $/kg and refrigerant-equivalent GWP data this pass did not go looking for. Did not attempt to reconcile the two literature anchors' differing spans (22.5K vs. ~1K) into a single calibrated span-dependent trend — the gap between them is reported as an honest range, not resolved by an unsupported interpolation.

## Performance follow-up: fig33/fig34 stopped re-running steps 11b/11d's own NSGA-III comparisons — done

Motivation. A `python main.py` run log surfaced that step 12's figure generation (`core/plots.py`'s `run_all()`) was, for two figures specifically, silently redoing work the pipeline had already just done one or two stages earlier: `plot_hysteresis_sensitivity()` (fig33) called `hysteresis_sensitivity.run_hysteresis_sensitivity()` unconditionally, and `plot_magnet_geometry_pareto_sensitivity()` (fig34) called `optimize.run_optimization()` unconditionally **twice** (once per FLAT/GEOMETRIC variant) — even though step 11b and step 11d, run moments earlier in the same pipeline invocation, had already computed exactly these two ON/OFF and FLAT/GEOMETRIC A/B comparisons. Every other "duplicate an earlier stage" figure (fig08, fig14, fig16, fig18, fig19/20, fig21, fig25, fig26) already followed the established `precomputed` dict pattern (see `run_all()`'s own docstring) to avoid this; fig33/fig34 were simply never updated to match when they were added in Phases 16/19.

Fix. `plot_hysteresis_sensitivity(precomputed=None)` and `plot_magnet_geometry_pareto_sensitivity(precomputed=None)` now accept the same `precomputed` dict every other reused-data figure does, reading new `'hysteresis_result'`/`'magnet_geometry_result'` keys (the exact dicts `run_hysteresis_sensitivity()`/`run_geometric_cost_pareto_sensitivity()` already return) when present, and falling back to a fresh computation — identical to the pre-fix behavior — when absent (including the existing pymoo-unavailable CSV-fallback path, untouched). `run_all()`'s figure-function list was updated from bare function references to `lambda: plot_x(precomputed)`, matching every other reused-data entry in that list (fig33/fig34 were the only two still passed as bare references, which is also why they'd silently never picked up `precomputed` even if a caller supplied it). `main.py`'s step 12 dispatch now passes `hysteresis_result` and `magnet_geometry_result` (both already captured by steps 11b/11d for the executive summary) into that same `precomputed` dict.
