...
4-point fit. Running `run_extended_diagnostic()` (as it stood in Phase 6,
plain unconstrained least squares with negative coefficients clipped to
zero afterwards) confirmed this made things *worse*, not better:
...
Leave-one-out cross-validation on the 4-point set gave errors from +2.6%
(Astronautics) up to **+1638.9%** (Tušek, the smallest device, predicted
from the other three which span up to 2502 W).

**Phase 7 revisited this with the correct solver, and then tested (and
rejected) the size-term hypothesis.** Unconstrained least squares plus
post-hoc clipping doesn't minimize anything under the actual constraint
(coefficients ≥ 0); re-solving with non-negative least squares (NNLS,
...
generalize, and a size/scale term isn't the fix the Phase 6 write-up
speculated — heterogeneous drivetrain/component-efficiency class across
devices is the more plausible driver, and more benchmark devices with
independently reported component efficiencies is the concrete next step.
The `CORE` 3-point fit remains the default used everywhere else in the
codebase (`amr_cycle.py`, `cascade.py`, `optimize.py`,
`giant_mce_analysis.py`); the EXTENDED fit and both diagnostics are
exposed only via `run_extended_diagnostic()` for transparency. A 5th
candidate device (Risø/DTU 2011, 30 K span) couldn't even be calibrated —
its reported 1.1 T field can't reach positive Qc at that span in this
model, independent corroboration of the single-stage span-collapse
