...
*Corrections
to earlier drafts are marked **CORRECTED**; newer entries are marked
**NEW**. Where this project's own copies of the
...
* **Phan & Yu**, *Journal of Magnetism and Magnetic Materials*, **308**, 325–340 (2007).
  ...
  * Discuss material selection criteria including entropy change, hysteresis, thermal conductivity and operating temperature. La₀.₇Ca₀.₃MnO₃, from this materials class, is the third candidate `core/passive_regenerator_analysis.py`'s passive-regenerator comparison uses precisely because its Curie point (267 K) sits *outside* the ASHRAE operating window — the "zero-gain control case" that confirms the passive-regenerator benefit is a genuine Tc-alignment effect and not a fixed per-material bonus (see Section 2's Passive/Hybrid Regenerators entry).
...
* **Zhang et al.**, arXiv:2312.09341, "Giant magnetocaloric effect and hysteresis loss in MnₓFe₂₋ₓP₀.₅Si₀.₅ (x=0.7-1.2) microwires."

  * Reports a directly-tabulated hysteresis-loss-vs-composition curve (W_hys, J/kg, integrated M-H loop area) for a closely-related Fe₂P-type Mn-Fe-P-Si system: 19.6 J/kg at x=0.8 (Tc=351K), rising to 60.7 J/kg at x=0.9 (Tc=298.5K), then falling to 28.9 J/kg at x=1.2 (Tc=190K) — non-monotonic in composition, not a simple trend `core/first_order_mce.py`'s Tc-only tuning could interpolate.
  * This is the ONLY source in this repo's corpus that reports hysteresis loss in J/kg (rather than a K-valued thermal hysteresis width) for anything in the (Mn,Fe)₂(P,Si) family, which is why `MNFEPSI_FIRST_ORDER.hysteresis_loss_J_per_kg=25.0` is calibrated against it as a same-family proxy rather than the Hanggai et al. (2026) composition actually used for Tc/ΔS_M — see the block comment above `MNFEPSI_FIRST_ORDER` in `core/first_order_mce.py` for the full honesty flag, and `core/hysteresis_sensitivity.py` for how much this placeholder actually moves the multi-objective optimizer's own material ranking.

* **NEW — hysteresis-reducing dopant routes not yet reflected in this repo's placeholder.** A separate line of (Mn,Fe)₂(P,Si)/Fe₂P-type literature, found by a targeted search for "does anything in this family have a fundamentally better ΔS-vs-hysteresis tradeoff than what's already modeled," reports several substitution routes that cut thermal hysteresis by roughly an order of magnitude relative to the un-doped compositions Zhang et al. (above) tabulate, while keeping the giant MCE intact:

  * **Lai, Huang, You, Maschek, Zhou, van Dijk & Brück**, *J. Sci. Adv. Mater. Dev.* **9**(1), 100660 (2024) — V-substitution at fixed 0.02 in (Mn₁.₁₇₋ₓFe₀.₇₃₋ᵧV₀.₀₂)(P₀.₅Si₀.₅) drives thermal hysteresis down to **0.6 K** (V-on-Fe substitution) while keeping ΔS_M ≈ 9.2 J/(kg K) at 1 T and ΔT_ad comparable to Gd; V substitution reduces the transition's volumetric strain, which the paper links directly to the hysteresis reduction.
  * **Lai, Huang, Bessas, You, Maschek, Zeng, Zhang, van Dijk & Brück**, arXiv:1810.09902 (*Mn₁₋ₓVₓFe(P,Si,B)*) — an earlier V-on-Mn substitution route reports an even lower **0.7 K** thermal hysteresis with |ΔS_M|=5.6 J/(kg K), ΔT_ad=2.3 K at 1 T, attributing the drop to V lowering the transition's latent heat.
  * **PMC5344570** (Co-B/Ni-B co-doped (Mn,Fe)₂(P,Si), *Materials* **10**, 14 (2017)) — B co-doping reports thermal hysteresis falling from 30.0 K (undoped) to 17.0 K, 1.5 K, and **0.0 K** (not experimentally resolvable) as B content rises from z=0.00 to 0.06, at an average rate of ~7 K per at.% B.

  **Why this is flagged rather than modeled:** none of these three sources report hysteresis loss in J/kg (the unit `hysteresis_loss_J_per_kg` and `core/amr_cycle.py`'s `W_hys = hysteresis_loss_J_per_kg × mass × frequency` actually need) — only thermal hysteresis width in K and, for two of the three, ΔS_M/ΔT_ad separately. Converting a hysteresis *width* (K) to a loop-area *energy* (J/kg) needs the transition's latent heat or a digitized M-H loop, neither of which is in this repo's corpus for these specific doped compositions. Inventing that conversion here — e.g. by naively scaling Zhang et al.'s 25.0 J/kg placeholder down by the same ratio the K-values shrink by (25 × 0.6/typical-undoped-ΔT_hys) — would be exactly the kind of backsolved, uncited number this codebase's own conventions exist to avoid, so no new `hysteresis_loss_J_per_kg` value is set from this finding. What IS a legitimate, non-fabricated takeaway: `core/hysteresis_sensitivity.py`'s own two-point A/B bracket (25.0 J/kg "ON" vs. 0.0 J/kg "OFF") already spans this literature's real-world range better than either endpoint suggests in isolation — the "OFF" (0.0 J/kg) case is no longer a purely hypothetical null hypothesis; it is now within-reach of demonstrated, giant-MCE-preserving doped compositions (down to 0.0 K measured thermal hysteresis), which strengthens the "does hysteresis change the ranking at all" reading of that module's existing sensitivity result rather than requiring a new one. Closing this gap for real would need a targeted re-read of these three papers' own supplementary M-H loop data (if published) or a fourth paper reporting J/kg directly for a V- or B-doped composition — flagged as a concrete next step in `ROADMAP.md` rather than left implicit.

  **Phase 36 speculative estimate (not a new default):** `core/mnfepsi_doped_hysteresis_speculative.py` takes a different, still-non-fabricated route to a J/kg number — rather than inventing a K-to-J/kg conversion out of thin air, it fits a dimensionless proxy k (W_hys ≈ k · ΔS · T_hys) against Zhang et al.'s own five measured x=0.8–1.2 points (mean k=0.151, std 18.6%; least-squares-through-origin k=0.162), then applies that fitted k to the two V-doped compositions above. Both come out well under 1 J/kg (0.6–0.9 J/kg, mean-k and least-squares-k agreeing within ~10% of each other) — consistent with, but not more precise than, the qualitative "close to an order of magnitude below the 25.0 J/kg placeholder" conclusion already stated above. This is flagged, in the module's own docstring, as a far extrapolation (T_hys ~20× below the fitted range) across a different composition axis (V-doping introduces a third element with its own, separately-cited mechanism) using data at 1 T rather than the model's own 2 T reference field — three compounding caveats, not one. `MNFEPSI_FIRST_ORDER.hysteresis_loss_J_per_kg` is unchanged; this module is not wired into any pipeline.
...
* **Bjørk, Bahl, Smith, Christensen & Pryds**, *International Journal of Refrigeration*, **34**, 1805–1816 (2011). ("Determining the minimum mass and cost of a magnetic refrigerator")
  ...
  * The primary source behind `core/economics.py`'s `COST_MCM_PER_KG` ($20/kg) and `COST_MAGNET_PER_KG` ($40/kg, NdFeB N42) constants and the magnet-to-MCM mass-ratio approximation used in the earlier flat-ratio cost model.
...
* **Gottschall et al.**, *Nature Materials* **17**, 929–934 (2018), "A multicaloric cooling cycle that exploits thermal hysteresis."

  * A genuinely different DEVICE ARCHITECTURE, not just a material swap: instead of minimizing the thermal hysteresis inherent to first-order magnetostructural materials (this repo's entire MNFEPSI/GD_FAMILY/LAFESIH/nanocomposite/hysteresis_sensitivity treatment, all of which treat hysteresis loss purely as a parasitic cost to reduce), this cycle deliberately *uses* hysteresis: a second stimulus (uniaxial stress) locks the field-induced ferromagnetic phase in place after the magnetic field is removed, so the field only needs to be applied briefly rather than held. Demonstrated in Ni-Mn-In Heusler alloys.
  * The claimed benefit is **not** a COP number — none is reported in this paper — but a drastic reduction in required permanent-magnet volume/mass (the field source no longer needs to stay engaged for the full cycle), which `core/economics.py` and `core/magnet_geometry.py` both treat as a major cost driver (magnet mass scaling super-linearly with field per the Bjørk/Halbach entries above).
  * A 2025 follow-up (**J. Appl. Phys. 137, 014901 (2025)**, TU Darmstadt, using FeRh with a purpose-built pulsed-field + uniaxial-stress test bench) confirms the same dual-stimulus mechanism generalizes beyond Ni-Mn-In, and separately notes the two individual caloric effects are "non-synergic" (the combination unlocks the *hysteresis-exploiting cycle*, not a larger ΔS/ΔT than either stimulus alone).
  * **Why this is documented rather than modeled**: this repo has no actuator-work or mechanical-stress-cycling model anywhere (the closest analog, `core/thermal_diode.py`'s mechanical-contact diode, is a heat-switch, not a stress actuator) and neither source paper reports an end-to-end device COP to calibrate one against — inventing a mechanical-actuator parasitic-loss model here would be the same fabrication trap flagged throughout this codebase's other honesty flags. What IS a legitimate, direct implication for THIS repo's own findings: `regime_crossover_analysis.py`'s null result and `magnet_geometry.py`'s cost model are both built entirely around the "hold the field on for the whole cycle" AMR assumption every material family and cascade design in this repo shares; this architecture is a structurally different point in design space (mechanically, not just materially) that neither of those checks searched, and closing it would need new mechanical-actuator work-cost data this repo's corpus does not have — flagged as a concrete, scoped-but-unstarted item in `ROADMAP.md` rather than left implicit.

  * **Phase 36 speculative estimate (still not a COP):** `core/hysteresis_exploiting_actuator_estimate.py` estimates the one new number this architecture's physics actually introduces — the specific mechanical work (J/kg) to strain the material through the stress-induced transformation — using Pfeuffer et al. (2021)'s own reported 55 MPa stress (real, measured) times an ASSUMED transformation-strain range of 0.3–1.0% (bracketing, not read directly from, the paper's separately-reported 0.75% *permanent* strain at the adjacent 75 MPa point) over an assumed 8000–8600 kg/m³ Ni-Mn Heusler density range. The resulting floor, 19–69 J/kg per stress half-cycle, sits in the same order of magnitude as this repo's own existing `hysteresis_loss_J_per_kg` values (2–65 J/kg, `core/first_order_mce.py`) — i.e. this new parasitic channel is not obviously negligible next to what AMR already pays for hysteresis. This is a thermodynamic-minimum floor only: it excludes actuator/hydraulic drive inefficiency entirely (no such figure was located for this application, so none is assumed), and it is a per-cycle specific work, not a device power or COP — no cycling frequency or device mass is assumed. Not wired into any pipeline; changes no existing result.
...
* **Kitanovski et al.**, *Magnetocaloric Energy Conversion*, Springer (2015).
  ...
  * **Consolidated book-access note** (see Section 6 for the full accounting): this project's own copy is a ~30-page front-matter/Ch. 1 excerpt. Chapters/sections cited by the phase plan but not present in this copy, each separately confirmed rather than assumed: Ch. 3 (Magnetic Field Sources, incl. Sect. 3.4/3.5 on Halbach cylinders), Ch. 4 (AMR performance), Ch. 6 (thermal diodes/heat switches), Ch. 7 (prototypes by country), Ch. 9 (system costs), and the fluids/ferrofluid chapter.
...
#### Elastocaloric Cooling — Comparison Reference **[NEW]**
...
* **Bjørk, Bahl & Smith**, *International Journal of Refrigeration*, **34**, 1805–1816 (2011).
  ...
  * See the full entry and correction note under Section 2's *Permanent Magnet Design* — this is the primary source behind `economics.py`'s per-kg cost constants and its `bom_cost()` full-system BOM extension (soft-magnetic yoke term added per Silva et al. 2017's methodology, below).
...
#### Full-System Cost Estimation **[NEW]**
...
* **Kitanovski, Tušek, Tomc, Plaznik, Ožbolt & Poredoš**, *Magnetocaloric
  ...
  plus ~9 pages of Ch. 1 only. Sections cited during development but absent
  from this copy: Ch. 3 (incl. Sect. 3.4/3.5, Halbach cylinders), Ch. 4
  (AMR performance, Sect. 4.1.1-4.1.4 cycle-topology relations), Ch. 6
  (special heat-transfer techniques / thermal diodes), Ch. 7 (prototypes
  by country), Ch. 9 (system costs), and its fluids-chapter content.
...
* **Tishin & Spichkin**, *The Magnetocaloric Effect and its Applications*,
  ...
  / Tc-distribution broadening), Sect. 2.9 and Ch. 10
  (superparamagnetic / nanocomposite materials), Ch. 9
  (amorphous / melt-spun-ribbon materials), and Ch. 11
  (regenerator/hybrid-cycle content).
...
this repo's corpus (the amorphous-material cost/performance
...
* Although first-order magnetocaloric materials exhibit larger entropy
  ...
  asserted: the literature-analog hysteresis-loss placeholders,
...
* Few studies integrate **material selection, AMR thermodynamics,
  ...
  *(Now largely addressed by this repo's own NSGA-III
  material+geometry co-optimization, extended with a
...
1. **Material-level validation** using the experimental magnetocaloric
   ...
   **direct** ΔTₐd measurement and Pecharsky & Gschneidner (1997)'s
   heat-capacity-derived peak ratio (Sections 1 above). A Gaussian Tc-broadening sensitivity check additionally probes how
...
2. **System-level validation** against an experimental benchmark set
   ...
   including the figure-digitized curve check above); Okamura &
   ...
   2013's digitized Figs. 10-11) for the device that has one, and a
   directional cycle-topology (Ericsson- vs. Brayton-like)
