...
|---|---|---|---|
| Particle diameter `d_p` | 0.5 mm default (sweep range 0.05–2 mm) | Sphere/cylinder radius in geometry | §3.1 — an earlier geometry sweep found COP-optimal packed-bed diameter at 0.5 mm for the 291K/10K-span/2kg/1Hz/1.5T operating point; **not necessarily optimal** at the 1.13T/10.2K-span/1.7kg/0.75Hz benchmark point used for the Step 14 degeneracy check — re-sweep if geometry itself is the research question |
...
| Quantity | Value | Where it's used | Source in original guide |
|---|---|---|---|
| Plate spacing (fluid gap) | 0.1–0.2 mm typical; earlier COP optimum found at **0.1 mm** at the 291K/10K operating point | Channel gap dimension in geometry | §3.2 — `core/geometry_analysis.py` |
...
2. Right-click **Geometry 1 → Rectangle** (fluid domain):
   ...
   - **Height** (gap): `0.0001 m` (0.1 mm, the earlier-found optimum) — or `0.0002 m` if using the "typical" upper end of the 0.1–0.2 mm range.
   - **Position**: `(0, 0)`.
