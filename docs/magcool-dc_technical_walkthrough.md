...
order the physics and design logic actually build up. It supersedes
earlier walkthroughs. Every numeric result below was re-verified against
the actual code during this revision (437/437 tests passing; individual
pipeline stages spot-checked directly rather than via one full
`python main.py` run, since the layered-bed NSGA-III stage alone can take
several minutes even at its own reduced pipeline settings — see
Section 13.3); every section states what's new, what changed, and —
carried over from this document's own established convention — what
still doesn't work, rather than smoothing that over.

**How to read the honesty flags below:** several later modules were
built against two reference books — Kitanovski et al. (2015),
...
**Confirmed this is a genuine dead end, not an open TODO.**
...
### 1.7 Inhomogeneous / polycrystalline Tc-broadening — `inhomogeneous_broadening.py`
...
flagged in Section 8 below), so what's implemented is
...
### 2.2 Engineered multi-phase nanocomposite — `nanocomposite_material.py`
...
### 2.3 Ga1-xCMn3+x antiperovskite family — `antiperovskite_material.py`
...
### 2.4 Mn1-xCuxCoGe magnetostructural family — `first_order_mce.py`
...
### 2.5 Latent-heat Cp spike — `first_order_mce.py`
...
Carnot-work / two-COP equations are the foundation. Every later addition adds
...
- **`cycle_type`** (Section 7's sensitivity check): `"brayton"`
...
### 5.3 Geometry-explicit intragranular eddy-current loss — `thermal.py`, `loss_model.py`
...
### 5.4 Pump/motor efficiency — `amr_cycle.py`
...
### 7.1 Cycle-topology sensitivity — `run_cycle_type_validation()`
...
### 8.1 Passive/hybrid regenerator augmentation
...
### 8.2 Elastocaloric reference line
...
**Constant-loss mode** (`results/sobol_results_constant_losses.txt`):
...
conclusion (Section 16). Both newer candidates fall back to Gd
...
### 13.1 Does thermal-hysteresis loss change the material-selection result?
...
### 13.2 Magnet-geometry (Halbach) cost-term sensitivity
...
part of a later thoroughness review): `cost_index()`'s own
...
### 13.3 Layered/graded beds exposed to NSGA-III — `optimize.py`, `cascade.py`
...
earlier call exactly) so a layered search has access to the same
...
### 15.1 Materials-cost floor and full-system estimate
...
### 15.3 Geometric Halbach-cylinder magnet mass
...
the missing nonlinearity flagged as absent from the earlier model;
its effect on the NSGA-III material front is Section 13.2 above.

### 15.4 Amorphous-material cost/performance note (qualitative only)
...
Single-stage AMR vs. VCC vs. liquid cooling vs. Carnot (and, since the
...
2. The hysteresis-on-vs-off material-selection reversal (Section
